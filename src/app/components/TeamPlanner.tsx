...
import { teamMembers, plannerTasks, categories } from './mockData';
...
export function TeamPlanner({ isReadOnly = false, memberCategoryNames = [] }: TeamPlannerProps = {}) {
  ...
  const getTaskLeft = (task: typeof plannerTasks[0]) => (task.startHour - HOURS[0]) * COL_WIDTH;
  const getTaskWidth = (task: typeof plannerTasks[0]) => (task.endHour - task.startHour) * COL_WIDTH - 6;
  const getMemberTasks = (memberId: number) => tasks.filter(t => t.memberId === memberId);

  const handleDrop = (memberId: number, hour: number) => {
    if (isReadOnly || dragging === null) return;
  ...
  const visibleMembers = teamMembers.filter(m => {
    ...
    return getMemberTasks(m.id).some(t => t.title.toLowerCase().includes(q));
  });

  const taskMatchesSearch = (task: typeof plannerTasks[0]) => {
    if (!q) return true;
    return task.title.toLowerCase().includes(q) || teamMembers.find(m => m.id === task.memberId)?.name.toLowerCase().includes(q);
  };
    ...
    <div className="p-6">
        ...
        <div>
          ...
          <p style={{ color: '#4B5563', fontSize: '13px' }}>
            {isReadOnly
              ...
              : 'Visualiza y redistribuye la carga de trabajo arrastrando los bloques.'}
          </p>
        </div>
        {/* Legend */}
        <div className="flex flex-wrap items-center gap-3 justify-end">
          {[{ color: ACCENT, label: 'Diseño UI' }, { color: '#7C6FE8', label: 'Backend' }, { color: '#10B981', label: 'Infraestructura' }, { color: '#F59E0B', label: 'Frontend' }, { color: '#EC4899', label: 'QA' }].map(({ color, label }) => (
        ...
        <div style={{ overflowX: 'auto', overflowY: 'visible' }}>
          <div style={{ minWidth: `${LEFT_WIDTH + TIMELINE_WIDTH}px` }}>
            ...
            {visibleMembers.map((member, memberIdx) => {
              const memberTasks = getMemberTasks(member.id).filter(taskMatchesSearch);
                    ...
                    <div style={{ display: 'flex', height: '80px' }}>
                      {HOURS.map((h, i) => (
                        <div key={h}
                          ...
                          onDragOver={e => { if (!isReadOnly) { e.preventDefault(); setDragOver({ memberId: member.id, hour: h }); } }}
                          onDrop={() => { if (!isReadOnly) handleDrop(member.id, h); }}
                          onDragLeave={() => { if (!isReadOnly) setDragOver(null); }}
                        />
                      ))}
                    </div>
                      ...
                      {memberTasks.map(task => {
                        ...
                        return (
                          <div key={task.id}
                            ...
                            onDragStart={e => { if (isReadOnly) return; e.stopPropagation(); setDragging(task.id); }}
                            ...
                            onMouseEnter={e => { if (!isReadOnly) e.currentTarget.style.boxShadow = `0 4px 16px ${task.color}30, 0 0 0 1px ${task.color}60`; }}
                            onMouseLeave={e => { if (!isReadOnly) e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,0.25)'; }}
                          >
            ...
            {visibleMembers.length === 0 && (
              <div style={{ padding: '40px', textAlign: 'center', color: '#374151', fontSize: '13px', fontStyle: 'italic' }}>
                No se encontraron miembros o tareas con "{search}".
              </div>
            )}
          </div>
        </div>
      ...
      {!isReadOnly && (
        <p style={{ color: '#374151', fontSize: '11px', marginTop: '10px', textAlign: 'center' }}>
          Arrastra los bloques para redistribuir tareas entre miembros del equipo.
        </p>
      )}
    </div>
  );
